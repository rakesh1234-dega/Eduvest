...
import {
  ...
  BarChart3, Settings, LogOut, ShieldCheck, Search, Bell, ChevronRight, Menu
} from "lucide-react";
...
export default function AdminLayout() {
          ...
          return (
            ...
            >
              <span className={cn(
                ...
                <span className={cn(isActive ? "text-white font-bold" : "text-slate-300")}>{item.label}</span>
                {isActive && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-indigo-400 shrink-0" />}
            </NavLink>
          );
        })}
      </nav>
  ...
  return (
    <div className="min-h-screen flex bg-slate-950 text-slate-200">
      {/* ── Sidebar (Desktop) ── */}
      <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shrink-0 hidden lg:flex h-screen sticky top-0">
        <SidebarContent />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Navbar */}
        <header className="h-16 bg-slate-900/80 backdrop-blur-sm border-b border-slate-800 flex items-center px-4 md:px-6 gap-4 shrink-0 transition-all">
          {/* Mobile Menu Trigger */}
          <Sheet>
            <SheetTrigger asChild>
              <button className="h-9 w-9 rounded-xl flex items-center justify-center lg:hidden hover:bg-slate-800 transition-colors">
                <Menu className="h-5 w-5" />
              </button>
            </SheetTrigger>
            <SheetContent side="left" className="p-0 w-64 bg-slate-900 border-r-slate-800 text-slate-200">
          ...
          <div className="flex items-center gap-2 bg-slate-800 rounded-xl px-4 py-2.5 text-sm text-muted-foreground flex-1 max-w-sm hidden md:flex">
          ...
          <div className="ml-auto flex items-center gap-4">
            <button
              ...
              className="hidden sm:flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors border border-slate-700"
            >
